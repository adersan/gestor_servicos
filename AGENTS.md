...
- Ainda nao publicado, em andamento: usuario pediu pra levar o modelo de etapas (wizard) do `#serviceDialog` para TODOS os modais do sistema no mobile/tablet. Plano de fases salvo (rollout dialog a dialog, testando antes de avançar).
  ...
  - **Convertido, aguardando teste do usuario**: `supplierEntryDialog` (Lançamento direto do fornecedor, 8 passos) — Fornecedor e Serviço agora usam picker de cards (reaproveitando `pickerSuppliers`/`pickerServicesForSupplier` ja existentes), igual ao `serviceDialog`.
