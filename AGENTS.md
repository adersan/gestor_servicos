...
- Ainda nao publicado: barra de navegacao inferior mobile (`.mobile-bottom-nav`) aumentada — botoes de `min-height:56px`/icone `20px`/texto `10px` para `min-height:74px`/icone `27px`/texto `13px`, com mais espacamento (`padding:10px`, `border-radius:16px`). Referencia visual passada pelo usuario (`notao-grande.jpeg` na raiz, nao versionado — modelo de app bancario com botoes grandes) comparada ao estado anterior (`botao-pequeno.jpeg`). Ajustado junto: `main` padding-bottom (92px→112px) e a posicao do botao flutuante `+` (`bottom:97px`) pra acompanhar a barra mais alta.
