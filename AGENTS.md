...
- Ainda nao publicado: botao flutuante `+` (`#newEntryFab`, classe `.new-entry-fab` em `index.html`/`styles.css`) fixo no canto inferior direito, visivel em TODAS as paginas (desktop e mobile/tablet) — abre o mesmo `serviceDialog` de sempre reaproveitando o atributo `data-dialog="serviceDialog"` ja tratado pelo listener delegado generico em `app.js`, sem JS novo. Posicionado acima do toast (`#appToast`, tambem fixo no canto inferior direito) no desktop, e acima da `.mobile-bottom-nav` no mobile/tablet; some quando o menu mobile esta aberto (`body.mobile-menu-open .new-entry-fab{display:none}`), igual a barra inferior.
