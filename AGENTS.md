...
- Ainda nao publicado: modal de lancamento (`#serviceDialog`) ganhou um fluxo de perguntas por etapa no mobile (`<=700px`), so para lancamento novo (inclusive "Importar pedido") — editar um lancamento existente continua abrindo o formulario completo de uma vez, como sempre. No desktop nada mudou. Cada etapa e um agrupamento do mesmo DOM/campos de sempre (`.wizard-step` em `index.html`), sem novo caminho de salvamento: os botoes Sim/Nao (solicitante, complementares, fornecedor), Hoje/Outra data e os 3 botoes de Situacao so manipulam os inputs reais e disparam `change`, reaproveitando `toggleServiceRequesterSection`/`toggleAdditionalServices`/o listener de `hasSupplierService` em `supplier.js` que ja existiam. Etapa final mostra um resumo somente leitura e aciona o mesmo botao `button[value="default"]` do submit de sempre (`app.js` linha ~3443, controlador novo perto de `openEntryForm`, funcoes `setServiceWizardMode`/`goToServiceWizardStep`/`validateServiceWizardStep`/`renderServiceWizardSummary`). Enter dentro do modo etapas aciona "Proximo/Salvar" em vez do salto entre campos ocultos (`focusNextFrom`) usado no desktop, que continua intacto.
