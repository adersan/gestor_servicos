...
- Ainda nao publicado, em andamento: usuario pediu pra levar o modelo de etapas (wizard) do `#serviceDialog` para TODOS os modais do sistema no mobile/tablet. Plano de fases salvo (rollout dialog a dialog, testando antes de avançar). Fase 0 concluida: extraido `createDialogWizard(config)` em `app.js` (funcao generica reaproveitavel — navegacao entre etapas com pulo condicional, barra de progresso, foco, botoes de escolha curta via `data-choice-for`/`data-choice-value` genericos, botao Hoje/Outra data via `data-date-choice`) SEM tocar no codigo especifico ja existente do `serviceDialog` (zero risco de regressao no que ja estava aprovado — o `serviceDialog` continua usando suas proprias funcoes `setServiceWizardMode`/`goToServiceWizardStep`/etc., nao foi migrado). Duas regras CSS novas genericas (`.wizard-mode .wizard-step label`/`input,select,textarea` e `.wizard-mode .dialog-actions,.dialog-form-actions`) foram adicionadas AO LADO das regras especificas de `#serviceForm` (nao substituindo — cuidado ao mexer aqui, remover a regra especifica de `#serviceForm` quebraria a especificidade CSS e faria `#serviceDialog label{gap:5px}` ganhar de volta). Fase 1 (piloto): `paymentDialog` ("Registrar pagamento") convertido usando o controlador generico — 6 passos (Cliente/Data/Valor/Forma de pagamento/Observacao/Confirmacao), botoes de forma de pagamento gerados dinamicamente a partir das `<option>` do select real (nao lista fixa), reaproveita a funcao `syncPaymentClientSelection` e o submit handler ja existentes sem duplicar logica. Usuario testou o paymentDialog e aprovou ("ficou bom"). `createDialogWizard` ganhou suporte a campo checkbox no padrao `data-choice-for`/`data-choice-value` (Sim=1/Nao=0 seta `.checked` em vez de `.value` quando o campo e checkbox) pra poder reaproveitar o mesmo padrao de botao curto em toggles booleanos, nao so em select/texto. `billingDialog` ("Gerar cobrança") convertido em seguida — 5 passos (Cliente/Período com as 2 datas juntas/Formas de pagamento com os checkboxes existentes/Consulta de cobranças anteriores em Sim-Não/Confirmação); como esse dialog nao tem funcao de abertura dedicada (abre via o handler generico `data-dialog`), a ativacao do wizard foi conectada direto nesse handler (`billingWizard.activate(...)` antes do `showModal()`). **Aguardando o usuario testar o billingDialog antes de continuar** pras proximas fases (paymentMethodDialog, supplierDialog, supplierEntryDialog, supplierPayableDialog, depois os formularios pequenos, depois trackingDialog/supplierAccessDialog com painel de resultado condicional, clientDialog com abas fica pra decidir depois).
