...
- Ainda nao publicado, em andamento: usuario pediu pra levar o modelo de etapas (wizard) do `#serviceDialog` para TODOS os modais do sistema no mobile/tablet. Plano de fases salvo (rollout dialog a dialog, testando antes de avançar).
  ...
  - **Convertidos e testados/aprovados pelo usuario**: `paymentDialog` (Registrar pagamento, 6 passos), `billingDialog` (Gerar cobrança, 5 passos), `paymentMethodDialog` (Nova forma de pagamento, 6 passos).
  - **Convertido, aguardando teste do usuario**: `supplierDialog` (Novo fornecedor, 8 passos: Nome/WhatsApp/Destino das solicitações/Nome do grupo/CPF ou CNPJ/Observações/Fornecedor padrão/Confirmação). Instanciado dentro do IIFE de `supplier.js` (nao app.js) — `createDialogWizard` fica global via `app.js`, que carrega antes de `supplier.js` no `index.html`, entao funciona chamar de la.
  - **Faltam** (na ordem do plano): `supplierEntryDialog`, `supplierPayableDialog`, depois os formularios pequenos (Bucket B), depois `trackingDialog`/`supplierAccessDialog` (tem painel de resultado condicional, mais trabalhoso), `clientDialog` (tem abas, decidir depois se vale trocar por etapas) fica por ultimo/a combinar.
