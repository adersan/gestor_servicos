...
- Ainda nao publicado, em andamento: usuario pediu pra levar o modelo de etapas (wizard) do `#serviceDialog` para TODOS os modais do sistema no mobile/tablet. Plano de fases salvo (rollout dialog a dialog, testando antes de avançar).
  ...
  - **Convertidos e testados/aprovados pelo usuario**: `paymentDialog` (Registrar pagamento, 6 passos), `billingDialog` (Gerar cobrança, 5 passos), `paymentMethodDialog` (Nova forma de pagamento, 6 passos), `supplierDialog` (Novo fornecedor, 8 passos).
  - **Convertido, aguardando teste do usuario**: `supplierEntryDialog` (Lançamento direto do fornecedor, 8 passos: Fornecedor/Serviço/Data/Referência/Valor/Status (3 botões)/Observações/Confirmação). Fornecedor e Serviço ficaram como campo de busca simples (nao picker de botao como no serviceDialog) pra manter o ritmo do rollout — pode virar picker depois se o usuario quiser.
  - **Faltam** (na ordem do plano): `supplierPayableDialog`, depois os formularios pequenos (Bucket B), depois `trackingDialog`/`supplierAccessDialog` (tem painel de resultado condicional, mais trabalhoso), `clientDialog` (tem abas, decidir depois se vale trocar por etapas) fica por ultimo/a combinar.
