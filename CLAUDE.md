...
- Ainda nao publicado: barra de navegacao inferior mobile (`.mobile-bottom-nav`) aumentada — botoes de `min-height:56px`/icone `20px`/texto `10px` para `min-height:84px`/icone `40px`/texto `13px`, com mais espacamento (`padding:10px`, `border-radius:16px`) e gap icone-texto reduzido ao minimo (`gap:1px`, era `4px`, pedido explicito do usuario "sem colar"). Referencia visual passada pelo usuario (`notao-grande.jpeg` na raiz, nao versionado — modelo de app bancario com botoes grandes) comparada ao estado anterior (`botao-pequeno.jpeg`). Texto aprovado direto; icone pedido +50% numa segunda rodada (27px→40px). Ajustado junto: `main` padding-bottom (92px→122px) e a posicao do botao flutuante `+` (`bottom:107px`) pra acompanhar a barra mais alta.
